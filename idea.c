...
#include <stdlib.h>
#define max 5
int push(char stack[max][80], int *top, char data[80]) //This function 
pushes the item (flavour) to stack (ice-cream) 
{
 if (*top == max - 1)
 3
 return (-1);
...
int pop(char stack[max][80], int *top, char data[80]) //This function 
removes the topmost item (flavour) from the stack (ice cream) 
{
 if (*top == -1)
 return (-1);
 else
...
void display(char stack[max][80], int *top, char data[80]) //Displays 
the whole stack (ice cream)
{
 int i;
 for (i = *top; i >= 0; i--)
 {
...
int insq(char queue[max][80], int *rear, char data[80]) //Enqueues 
...
Parlour");
 ...
 printf("\n 2. Delete your name from a Queue if you don't want icecream");
...
printf("\tMENU CARD");
 printf("\n----------------------------\n");
 printf("\n Make your own ice-cream here by choosing the 
below options.... :)\n");
 printf("\n");
 ...
 if (replyst == -1)
 printf("\n You can't add more flavours to the ice-cream 
(Stack is Full).\n");
 else
 printf("'%s' flavour has been added to your ice-cream 
(Pushed in Stack).\n\n", nm);
 break;
 case 2: // pop
 reply = pop(stack, &top, nm);
 if (replyst == -1)
 printf("\n No flavours has been added to the ice-cream 
(Stack is Empty) \n");
 else
 printf("Removed (Popped) flavour of the ice-cream is : 
%s\n", nm);
 printf("\n");
 break;
 case 3: //display
 ...
 break;
 case 4:
 printf("Kindly make your payment...Hope you enjoyed the 
ice-cream, please visit us again!!!:)");
 exit(0);
 default:
